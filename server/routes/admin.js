...
router.get('/hr', async (req, res) => {
    try {
        ...
        const { rows } = await db.query(
            "SELECT id, username, full_name, email, phone, is_active, created_at FROM users WHERE role='hr' ORDER BY created_at DESC"
        );
        res.json(rows);
    } catch (err) { res.status(500).json({ error: 'Failed to fetch HR users.' }); }
});
...
router.post('/hr', async (req, res) => {
    try {
        ...
        const hashedPassword = bcrypt.hashSync(customPassword || 'hr123', 10);
        const { rows: [newUser] } = await db.query(
            `INSERT INTO users (username, password, role, full_name, email, phone, must_change_password)
             VALUES ($1,$2,'hr',$3,$4,$5,1) RETURNING id, username`,
            [username, hashedPassword, full_name, email || '', phone || '']
        );
        logAudit(db, req.user.id, req.user.username, 'CREATE_HR', `Created HR: ${username}`, req.ip);
        res.status(201).json({
            message: 'HR created successfully.',
            hr: { id: newUser.id, username: newUser.username, default_password: customPassword || 'hr123' }
        });
    } catch (err) {
...
router.put('/hr/:id', async (req, res) => {
    try {
        ...
        const db = getDb();
        await db.query(
            "UPDATE users SET full_name=COALESCE($1,full_name), email=COALESCE($2,email), phone=COALESCE($3,phone), updated_at=NOW() WHERE id=$4 AND role='hr'",
            [full_name || null, email || null, phone || null, req.params.id]
        );
        logAudit(db, req.user.id, req.user.username, 'UPDATE_HR', `Updated HR ID: ${req.params.id}`, req.ip);
        res.json({ message: 'HR updated successfully.' });
    } catch (err) { res.status(500).json({ error: 'Failed to update HR.' }); }
});

router.delete('/hr/:id', async (req, res) => {
    try {
        const db = getDb();
        await db.query("DELETE FROM users WHERE id=$1 AND role='hr'", [req.params.id]);
        logAudit(db, req.user.id, req.user.username, 'DELETE_HR', `Deleted HR ID: ${req.params.id}`, req.ip);
        res.json({ message: 'HR removed successfully.' });
    } catch (err) { res.status(500).json({ error: 'Failed to delete HR.' }); }
});
...
router.get('/stats', async (req, res) => {
    try {
        const db = getDb();
        const [usersR, accountantsR, adminsR, hrR, payrollsR, recentR] = await Promise.all([
            ...
            db.query("SELECT COUNT(*) as cnt FROM users WHERE role = 'hr'"),
        ...
        res.json({
            ...
            total_hr: parseInt(hrR.rows[0].cnt) || 0,
...
router.get('/mfa-codes', async (req, res) => {
    try {
        const db = getDb();
        // Automatically clean up expired codes before fetching
        await db.query("DELETE FROM admin_mfa_codes WHERE expires_at <= NOW()");

        const { rows } = await db.query(`
            SELECT m.id, m.user_id, m.otp_code, m.created_at, m.expires_at, 
                   u.username, u.full_name, u.role
            FROM admin_mfa_codes m
            JOIN users u ON m.user_id = u.id
            ORDER BY m.created_at DESC
        `);
        res.json(rows);
    } catch (err) {
        console.error('Fetch MFA codes error:', err);
        res.status(500).json({ error: 'Failed to fetch active MFA codes.' });
    }
});
